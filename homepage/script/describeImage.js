...
function describeImage() {
	var image_div = document.getElementById("image_slide");
	...
	var image_alts = document.getElementsByTagName("img");
	var _scrolling = setInterval("autoScroll()",2000);
	// append the image descriptor 
	image_desc.appendChild(desc_text);
	image_div.appendChild(image_desc);
	
	for (var i=1; i<image_alts.length; i++) {
		...
		image_alts[i].onmouseover = function() {
			...
			if (this.getAttribute("alt").substr(0,4) == 'http') {
